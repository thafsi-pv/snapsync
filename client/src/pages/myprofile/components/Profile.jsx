import React from "react";

function Profile() {
  return (
    <div
      id="MyProfileRoot"
      className="overflow-hidden bg-white relative flex flex-row justify-between w-full items-start pl-8 pr-56 py-10">
      <div className="self-end relative flex flex-col mb-5 gap-16 w-2/3">
        <div className="flex flex-row justify-between items-start ml-20 w-full ">
          <div className="w-1/3 m-4">
            <img
              src="https://res.cloudinary.com/dm4djc1b1/image/upload/v1696873124/gfvwiaz7pw8qn6wtgvws.avif"
              className="ml-px w-96 h-60 rounded-full"
            />
          </div>
          <div className="flex flex-col gap-8 w-2/3 items-start">
            <div className="self-stretch flex flex-row ml-px gap-6 items-start">
              <div className="flex flex-row mt-1 gap-3 w-[524px] items-start">
                <div className="text-2xl font-['Microsoft_Sans_Serif'] mt-2">
                  Abdul_ahad_desgins
                </div>
                <div className="flex flex-row gap-2 w-1/2 items-start">
                  <div className="bg-[rgba(217,_217,_217,_0.4)] flex flex-col justify-center w-1/2 h-10 items-center rounded-lg">
                    <div className="text-sm font-['Microsoft_Sans_Serif']">
                      Edit profile
                    </div>
                  </div>
                  <div className="bg-[rgba(217,_217,_217,_0.4)] flex flex-col justify-center w-1/2 h-10 items-center rounded-lg">
                    <div className="text-sm font-['Microsoft_Sans_Serif']">
                      View archive
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-row gap-2 w-20 shrink-0 items-start">
                <img
                  src="https://file.rendit.io/n/UOf4M54q9M1eGElp42md.svg"
                  id="Instagramthreads"
                  className="w-10 shrink-0"
                />
                <img
                  src="https://file.rendit.io/n/uXmNQrw03s12xLMCnK7x.svg"
                  id="Settingsline"
                  className="mt-2 w-8 shrink-0"
                />
              </div>
            </div>
            <div className="flex flex-col gap-6 w-3/5">
              <div className="flex flex-row gap-10 items-start">
                <div className="text-lg font-['Microsoft_Sans_Serif'] mr-1">
                  11 posts
                </div>
                <div className="text-xl font-['Microsoft_Sans_Serif']">
                  1M followers
                </div>
                <div className="text-xl font-['Microsoft_Sans_Serif']">
                  10M following
                </div>
              </div>
              <div className="flex flex-col gap-1 items-start ml-px mr-20">
                <div className="text-lg font-['Microsoft_Sans_Serif']">
                  queen_girl_00001
                </div>
                <div className="self-stretch flex flex-col gap-px items-start">
                  <div className="font-['Microsoft_Sans_Serif'] text-[#a7a1a1]">
                    UI/UX desginer
                  </div>
                  <div className="text-lg font-['Microsoft_Sans_Serif']">
                    Front end developer & Game Desginer
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="self-start flex flex-col gap-3 w-1/2 mb-1 ml-12">
          <div className="flex flex-row justify-between items-start">
            <div
              id="Ellipse1"
              className="bg-[url(https://file.rendit.io/n/5BqefhPtK3djYMI5vhZL.svg)] bg-cover bg-50%_50% bg-blend-normal bg-no-repeat flex flex-col items-start pt-px pb-1 px-px">
              <img src="" className="ml-px w-24" />
            </div>
            <div
              id="Ellipse3"
              className="bg-[url(https://file.rendit.io/n/5BqefhPtK3djYMI5vhZL.svg)] bg-cover bg-50%_50% bg-blend-normal bg-no-repeat flex flex-col items-start pt-px pb-1 px-px">
              <img src="" className="ml-px w-24" />
            </div>
            <div
              id="Ellipse4"
              className="bg-[url(https://file.rendit.io/n/5BqefhPtK3djYMI5vhZL.svg)] bg-cover bg-50%_50% bg-blend-normal bg-no-repeat flex flex-col items-start pt-px pb-1 px-px">
              <img src="" className="ml-px w-24" />
            </div>
            <div
              id="Ellipse5"
              className="bg-[url(https://file.rendit.io/n/5BqefhPtK3djYMI5vhZL.svg)] bg-cover bg-50%_50% bg-blend-normal bg-no-repeat flex flex-col items-start pt-px pb-1 px-px">
              <img src="" className="ml-px w-24" />
            </div>
          </div>
          <div className="flex flex-row gap-[114px] items-start ml-4 mr-6">
            <div className="font-['Microsoft_Sans_Serif']">Highlights</div>
            <div className="font-['Microsoft_Sans_Serif'] mr-2">Love</div>
            <div className="font-['Microsoft_Sans_Serif'] mr-1">Starts</div>
            <div className="font-['Microsoft_Sans_Serif']">Favone</div>
          </div>
        </div>
        <div className="flex flex-col gap-5">
          <div
            id="Line1"
            className="border-solid border-[#9b9b9b] mb-px h-px shrink-0 border-t border-b-0 border-x-0"
          />
          <div className="self-start flex flex-row ml-[315px] gap-16 items-start">
            <div className="flex flex-row mr-1 gap-px w-20 shrink-0 items-start">
              <img src="" className="mt-px w-6 shrink-0" />
              <div className="font-['Microsoft_Sans_Serif']">POSTS</div>
            </div>
            <div className="flex flex-row gap-2 w-20 shrink-0 items-start">
              <img src="" className="mt-px w-4 shrink-0" />
              <div className="font-['Microsoft_Sans_Serif']">REELS</div>
            </div>
            <div className="flex flex-row gap-2 w-20 shrink-0 items-center">
              <img
                src="https://file.rendit.io/n/LDe6sBzq16q9E7Pc61H1.svg"
                className="w-4 shrink-0"
              />
              <div className="font-['Microsoft_Sans_Serif'] self-start">
                SAVED
              </div>
            </div>
          </div>
          <div className="flex flex-row justify-between items-start">
            <img src="" />
            <img src="" />
            <img src="" />
          </div>
        </div>
      </div>
    </div>
  );
}

export default Profile;
